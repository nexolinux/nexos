...
export class RoundedCornersManager {
    ...
    global_rounded_corners = settings().global_rounded_corner_settings;
    custom_rounded_corners = settings().custom_rounded_corner_settings;
    // ---------------------------------------------------------- [public methods]
    on_add_effect(actor) {
        ...
        this._actor_to_rounded(actor)?.add_effect_with_name(constants.ROUNDED_CORNERS_EFFECT, new RoundedCornersEffect());
    ...
    on_size_changed(actor) {
        ...
        const effect = this._actor_to_rounded(actor)?.get_effect(constants.ROUNDED_CORNERS_EFFECT);
        ...
        effect.update_uniforms(UI.WindowScaleFactor(win), cfg, this._compute_bounds(actor, content_offset_of_win), {
            width: settings().border_width,
            color: settings().border_color,
        });
        // Update BindConstraint for shadow
        const shadow = window_info.shadow;
        const offsets = this._compute_shadow_actor_offset(actor, content_offset_of_win);
    ...
    on_focus_changed(actor) {
        ...
        this._update_shadow_actor_style(win, shadow, border_radius, shadow_settings, padding);
    }
    ...
    _create_shadow(actor) {
        ...
        this._update_shadow_actor_style(actor.metaWindow, shadow, this.global_rounded_corners?.border_radius, actor.metaWindow.appears_focused
            ? settings().focused_shadow
            : settings().unfocused_shadow);
        // We have to clip the shadow because of this issues:
        // https://gitlab.gnome.org/GNOME/gnome-shell/-/issues/4474
        shadow.add_effect_with_name(constants.CLIP_SHADOW_EFFECT, new ClipShadowEffect());
    ...
    _compute_bounds(actor, [x, y, width, height]) {
        ...
        if (settings().tweak_kitty_terminal) {
            ...
            if (actor.metaWindow.get_client_type() === type &&
    ...
    _compute_shadow_actor_offset(actor, [offset_x, offset_y, offset_width, offset_height]) {
        const win = actor.metaWindow;
    ...
    _update_shadow_actor_style(win, actor, border_radius_raw = this.global_rounded_corners?.border_radius, shadow = settings().focused_shadow, padding = this.global_rounded_corners?.padding) {
        ...
        const original_scale = St.ThemeContext.get_for_stage(global.stage).scaleFactor;
    ...
    _update_all_window_effect_state() {
        for (const actor of global.get_window_actors()) {
            const should_have_effect = this._should_enable_effect(actor.metaWindow);
    ...
    _update_all_shadow_actor_style() {
        for (const actor of global.get_window_actors()) {
            ...
            const shadow_cfg = actor.metaWindow.appears_focused
                ? settings().focused_shadow
                : settings().unfocused_shadow;
            const { border_radius, padding } = this._get_rounded_corners_cfg(actor.metaWindow);
            this._update_shadow_actor_style(win, shadow, border_radius, shadow_cfg, padding);
        }
    }
    _get_rounded_corners_cfg(win) {
        return UI.ChoiceRoundedCornersCfg(this.global_rounded_corners ??
            settings().global_rounded_corner_settings, this.custom_rounded_corners ??
            settings().custom_rounded_corner_settings, win);
    }
    /**
